...
async def dice_func(message: types.Message): await message.reply_dice()
...
async def process(message: types.Message): await bot_commands[message.get_command()[1:]][0](message)

@dp.message_handler(commands="start")
async def start_func(message: types.Message): await message.answer("Привет! Я помогу тебе сделать выбор в трудной ситуации. Попробуй написать /help")
...
async def album_handler(message: types.Message): await message.reply_photo(message.photo[-1].file_id)

@dp.message_handler(content_types=types.ContentType.ANY)
async def master(message: types.Message): await message.reply("Извини, я пока не могу с таким работать. Попробуй /help")

if __name__ == "__main__":
    while True:
        try: executor.start_polling(dp, skip_updates=True)
        except: time.sleep(5)